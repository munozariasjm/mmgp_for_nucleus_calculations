...
from nuclear_mmgp.src.modules.model_trainers.model_trainer import ModelTrainer
...
class MultiFidelityDeepGPTrainer(ModelTrainer):
    ...
    def __init__(self, data: TensorLike, num_outputs: int, optimizer_name: str):
        ...
        """
        super().__init__(
            data=data, optimizer_name=optimizer_name, num_outputs=num_outputs
        )
        X, Y = self.data

        tf.config.run_functions_eagerly(
            True
        )  # currently there's a bug where this needs to be enabled for gather_nd
        # This turns X and Y into ((num_examples x num_outputs) x 2) separated by labels
        self.Xs = [X[:, :-1]]
        self.Ys = [Y[:, :-1]]
        for i in range(self.num_layers - 1):
            self.Xs.append(
                tf.gather_nd(indices=tf.where(X[:, -1] > i), params=X[:, :-1])
            )
            self.Ys.append(
                tf.gather_nd(indices=tf.where(Y[:, -1] > i), params=Y[:, :-1])
            )
    ...
    def get_deep_kernel(self, kernel_names: str) -> List[gpf.kernels.Kernel]:
        ...
        for i in range(self.num_layers - 1):
            coreg = gpf.kernels.Coregion(
                ...
                active_dims=[self.num_dim],
            )
            coreg.W = np.random.rand(self.num_outputs, self.num_dim)
            kernels.append(
                (
                    self.get_kernel(kernel_names[0], active_dims=[self.num_dim + 1])
                    * (
                    ...
                    + self.get_kernel(kernel_names[0], active_dims=range_dim)
                )
                * coreg
            )
        return kernels
