...
class SharedIndependentTrainer:
    ...
    def __init__(self, Xs, Ys, inducing_size, optimizer=gpf.optimizers.Scipy()):
        ...
        self.length_scales = tf.convert_to_tensor(
            [1.0] * self.num_features, dtype=default_float()
        )

        self.num_outputs = self.Ys[0].shape[1]
        self.output_dims = list(
            range(self.num_features, self.num_outputs + self.num_features)
        )
        ...
        self.Z = np.tile(
            np.linspace(min_range, max_range, inducing_size)[:, None],
            (1, self.num_features),
        )
        self.Z_with_mean = np.tile(
            np.linspace(0, 1, inducing_size)[:, None],
            (1, self.num_features + self.num_outputs),
        )
        ...
        for fidelity in range(1, self.max_fidelity):
            kernel = gpf.kernels.SharedIndependent(
                gpf.kernels.SquaredExponential(
                    lengthscales=self.length_scales, active_dims=self.input_dims
                )
                * (
