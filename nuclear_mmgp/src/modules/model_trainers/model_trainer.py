...
from nuclear_mmgp.src.modules.models.deep_vgp import DeepVGP
from nuclear_mmgp.src.modules.models.multi_task_gp import MultiTaskGP
from nuclear_mmgp.src.modules.models.difference_model import DiffModel
from nuclear_mmgp.src.modules.models.multi_fidelity_gp import MultiFidelityGP

from nuclear_mmgp.src.kernels.neural_network_kernel import NeuralNetKernel
...
class ModelTrainer:
    ...
    def construct_model(self, model_names, base_kernel, likelihood_name):
        ...
        self.model = self.get_model(
            model_names, kernel=kernel, likelihood=likelihood, data=self.data
        )
    ...
    def get_kernel(
        self, kernel_name: str, active_dims: List[int] = None
    ) -> gpf.kernels.Kernel:
        ...
        if kernel_name == "NeuralNetwork":
            return NeuralNetKernel(
                base_kernel=gpf.kernels.RBF(),
                num_dims=self.num_dim,
    ...
    def get_multioutput_kernel(self, kernel_name: str) -> gpf.kernels.Kernel:
        coreg = gpf.kernels.Coregion(
            output_dim=self.num_outputs, rank=self.num_dim, active_dims=[self.num_dim]
        )
    ...
    def get_model(self, model_name, kernel, likelihood, data) -> gpf.models.GPModel:
        ...
        if model_name == "multi-fidelity-gp":
            return MultiFidelityGP(
                data, kernel=kernel, likelihood=likelihood, num_outputs=3
            )
        if model_name == "multi-task-gp":
            return MultiTaskGP(
                data, kernel=kernel, likelihood=likelihood, num_outputs=self.num_outputs
            )
        if model_name == "VGP":
            return gpf.models.VGP(
                data, kernel=kernel, likelihood=likelihood, num_latent_gps=1
            )
        ...
        if model_name == "DeepGP":
            ...
            return model
        raise Exception(
            f"Please enter a valid models name: one of {self.VALID_MODEL_NAMES}."
        )
    ...
    def get_optimizer(self, optimizer_name):
        ...
        raise Exception(
            f"Please enter a valid optimizer name: one of {self.VALID_OPTIMIZERS}"
        )
