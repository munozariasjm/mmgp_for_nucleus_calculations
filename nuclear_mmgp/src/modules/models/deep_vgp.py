...
class DeepVGP(gpf.models.VGP):
    def __init__(
        self, data: RegressionData, kernel: Kernel, likelihood: Likelihood, num_outputs
    ):
    ...
    def elbo(self) -> tf.Tensor:
        ...
        for i in range(self.num_layers):
            # Get conditionals
            K = (
                self.kernel(self.Xs[i])
                + tf.eye(self.Xs[i].shape[0], dtype=default_float()) * default_jitter()
            )
            ...
            L_tiled = tf.tile(
                tf.expand_dims(L, 0), tf.stack([self.num_latent_gps, 1, 1])
            )
