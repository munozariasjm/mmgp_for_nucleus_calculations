...
class DiffModel(gpf.models.GPModel, gpf.models.InternalDataTrainingLossMixin):
    ...
    def __init__(
        ...
        tf.config.run_functions_eagerly(
            True
        )  # currently there's a bug where this must be true for tf.gather_nd
        ...
        indices = np.random.choice(
            range(self.num_lf_data), size=self.num_hf_data, replace=False
        )
        self.X_lf_trunc = tf.concat(
            [
                tf.gather_nd(
                    indices=np.reshape(indices, (self.num_hf_data, 1)),
                    params=self.X_lo_fi[i : i + self.num_lf_data, :],
                )
                for i in range(3)
            ],
            axis=0,
        )
        self.Y_lf_trunc = tf.concat(
            [
                tf.gather_nd(
                    indices=np.reshape(indices, (self.num_hf_data, 1)),
                    params=self.Y_lo_fi[i : i + self.num_lf_data, :],
                )
                for i in range(3)
            ],
            axis=0,
        ...
        tf.config.run_functions_eagerly(
            False
        )  # change back to false for faster training
        ...
        self.rho = gpf.Parameter(
            tf.ones((int(self.num_hf_data * num_outputs), 1)),
    ...
    def get_variance(
        self, size: float, likelihood: gpf.likelihoods.Likelihood = None
    ) -> tf.Tensor:
        ...
        return tf.linalg.diag(
            tf.concat(
                [
                    tf.linalg.diag_part(tf.fill((n, n), variance))
    ...
    def maximum_log_likelihood_objective(self) -> tf.Tensor:
        ...
        var = hi_fi_var + tf.math.multiply(
            self.rho,
            self.get_variance(size=self.num_hf_data, likelihood=self.model.likelihood),
        )
        ...
        return -tf.reduce_sum(
            ...
            * (
                d_Kvar_d
                + log_det
                + tf.cast(self.num_hf_data * tf.math.log(2 * np.pi), dtype=tf.float64)
            )
        )
